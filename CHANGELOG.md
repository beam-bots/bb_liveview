<!--
SPDX-FileCopyrightText: 2025 James Harton

SPDX-License-Identifier: Apache-2.0
-->

# Change Log

All notable changes to this project will be documented in this file.
See [Conventional Commits](Https://conventionalcommits.org) for commit guidelines.

<!-- changelog -->

## [v0.3.4](https://github.com/beam-bots/bb_liveview/compare/v0.3.3...v0.3.4) (2026-09-12)




### Improvements:

* cancel any running command, not just ones the dashboard started (#150) by James Harton

## [v0.3.3](https://github.com/beam-bots/bb_liveview/compare/v0.3.2...v0.3.3) (2026-08-27)




### Bug Fixes:

* wire the parameter inputs to the server through forms (#136) by James Harton

* render and write unit-typed parameters as magnitudes (#132) by James Harton

## [v0.3.2](https://github.com/beam-bots/bb_liveview/compare/v0.3.1...v0.3.2) (2026-08-23)




### Bug Fixes:

* send multi-degree-of-freedom joint configurations as a pose (#130) by James Harton

## [v0.3.1](https://github.com/beam-bots/bb_liveview/compare/v0.3.0...v0.3.1) (2026-08-22)




### Improvements:

* tell the user when a joint refuses a slider command (#127) by James Harton

### Bug Fixes:

* deps: update phoenix_live_view to 1.2.9 (#120) by James Harton

## [v0.3.0](https://github.com/beam-bots/bb_liveview/compare/v0.2.9...v0.3.0) (2026-08-04)
### Breaking Changes:

* follow the `positions` to `configurations` rename in core (#113) by James Harton



## [v0.2.9](https://github.com/beam-bots/bb_liveview/compare/v0.2.8...v0.2.9) (2026-06-27)




### Bug Fixes:

* push the full joint pose to the 3D view, not one joint per frame (#86) by James Harton

## [v0.2.8](https://github.com/beam-bots/bb_liveview/compare/v0.2.7...v0.2.8) (2026-06-27)




### Bug Fixes:

* don't time out continuous commands; add cancel control (#85) by James Harton

### Improvements:

* throttle dashboard rendering under high-rate sensor traffic by James Harton

## [v0.2.7](https://github.com/beam-bots/bb_liveview/compare/v0.2.6...v0.2.7) (2026-05-17)




### Bug Fixes:

* render :atom-typed argument values with their leading colon (#45) by James Harton

* preserve command form input values after execute (#44) by James Harton

### Improvements:

* display event-stream timestamps from `BB.Message.wall_time` (#48) by James Harton

## [v0.2.6](https://github.com/beam-bots/bb_liveview/compare/v0.2.5...v0.2.6) (2026-05-16)




### Improvements:

* phoenix bootstrap + endpoint plug injection in `bb_liveview.install` (#43) by James Harton

## [v0.2.5](https://github.com/beam-bots/bb_liveview/compare/v0.2.4...v0.2.5) (2026-05-13)




### Improvements:

* add `bb_liveview.install` igniter task (#42) by James Harton

## [v0.2.4](https://github.com/beam-bots/bb_liveview/compare/v0.2.3...v0.2.4) (2026-01-18)



### Documentation:

*  add Diataxis-structured documentation (#10) by James Harton

## [v0.2.3](https://github.com/beam-bots/bb_liveview/compare/v0.2.2...v0.2.3) (2026-01-11)




### Bug Fixes:

* handle simulation mode and command completion in dashboard by James Harton

## [v0.2.2](https://github.com/beam-bots/bb_liveview/compare/v0.2.1...v0.2.2) (2026-01-09)




### Bug Fixes:

* use BB.Actuator.set_position! for direct actuator commands by James Harton

## [v0.2.1](https://github.com/beam-bots/bb_liveview/compare/v0.2.0...v0.2.1) (2026-01-09)




### Bug Fixes:

* update to support BB 0.13+ command API (#8) by James Harton

* sometimes crash parsing floats by James Harton

## [v0.2.0](https://github.com/beam-bots/bb_liveview/compare/v0.1.0...v0.2.0) (2025-12-20)




### Features:

* implement BB LiveView dashboard with 3D robot visualisation by James Harton


